...
// js目录下文件处理
gulp.task('script_file', function() {
     gulp.src(path.dev.js + '**/!(*.js)')//选择所有文件
        .pipe(gulp.dest(path.distDev.js))//输出线上备份文件
        .pipe(gulp.dest(path.distMini.js))//输出线上压缩文件
});
...
// 默认任务
gulp.task("default", ['script', 'image', 'css','lib','common','framework','script_file']);
