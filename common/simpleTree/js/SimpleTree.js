	...
	$.fn.extend({
		SimpleTree:function(options){
			...
			option._init=function(){
				...
				this.tree.find("ul ul").prev("li").find("table td:eq(0)").removeClass("open");	/* 移除所有子级菜单父节点的 open 样式 */
				
				this.tree.find("ul ul[show='true']").show();	/* 显示 show 属性为 true 的子级菜单 */
				this.tree.find("ul ul[show='true']").prev("li").find("table td:eq(0)").addClass("open");	/* 添加 show 属性为 true 的子级菜单父节点的 open 样式 */
			}/* option._init() End */
			...
			/* 设置所有父节点样式 */
			this.find("ul").prev("li").find("table td:eq(0)").addClass("folder");
